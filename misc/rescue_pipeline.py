...
sub_unkno = unkno[
    (unkno.AGATCTCGGT_dist <= 3)
    & (unkno.CTCTCTAC_dist <= 4)
    & (unkno.CGTACTAG_dist > 4)
]
...
def extract_rescue(indir, sample, part, limit):
    ...
    for bc in target_bcs:
        target_fastqs[f"R1_{bc}"] = R1_fastq.replace(
            f"split/{sample}", bc_sample_dict[bc] + "_rescue"
        )
        target_fastqs[f"R2_{bc}"] = R2_fastq.replace(
            f"split/{sample}", bc_sample_dict[bc] + "_rescue"
        )
    ...
    if os.path.isfile(target_fastqs[f"R1_{bc}"]):
        ...
        for bc in target_bcs:
            target_ios[f"R1_{bc}"] = open(
                R1_fastq.replace(f"split/{sample}", bc_sample_dict[bc] + "_rescue"), "w"
            )
            target_ios[f"R2_{bc}"] = open(
                R2_fastq.replace(f"split/{sample}", bc_sample_dict[bc] + "_rescue"), "w"
            )
            ...
            for r1, r2 in tqdm(zip(R1, R2)):
                ...
                for bc in target_bcs:
                    if idx_pair in accept_bcs[bc]:
                        write_fastq_pair_rescue(
                            target_ios[f"R1_{bc}"], target_ios[f"R2_{bc}"], r1, r2
                        )
...
for bc in target_bcs:
    subprocess.call(
        f"cat {indir}/{sample}/{bc_sample_dict[bc]}_rescue_R1.part*.fastq.gz > {indir}/{bc_sample_dict[bc]}_rescue_R1_001.fastq.gz",
        shell=True,
    )
    subprocess.call(
        f"cat {indir}/{sample}/{bc_sample_dict[bc]}_rescue_R2.part*.fastq.gz > {indir}/{bc_sample_dict[bc]}_rescue_R2_001.fastq.gz",
        shell=True,
    )
